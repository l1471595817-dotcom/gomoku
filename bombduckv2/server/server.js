...
wss.on("connection",ws=>{let pd=null;
  ws.on("message",raw=>{let d;try{d=JSON.parse(raw);}catch(e){return;}
    switch(d.t){
      ...
      case"bomb":{if(pd)bc(pd.room,{t:"bomb",id:pd.id,gx:d.gx,gy:d.gy,rg:d.rg},ws);break;}
      case"exp":{if(pd)bc(pd.room,{t:"exp",gx:d.gx,gy:d.gy,rg:d.rg},ws);break;}
      case"hit":{if(pd)bc(pd.room,{t:"hit",tgt:d.tgt},ws);break;}
      case"destroy":{if(pd)bc(pd.room,{t:"destroy",gx:d.gx,gy:d.gy},ws);break;}
      case"banana":{if(pd)bc(pd.room,{t:"banana",id:pd.id},ws);break;}
...
server.listen(PORT,"0.0.0.0",()=>console.log("BombDuckV2 WS on",PORT));